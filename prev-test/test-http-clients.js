...
class NaverSmartStoreHttpTest {
  ...
  async testHttpClient() {
    ...
    try {
      ...
      console.log(
        `   Content-Length: ${
          storeResponse.headers['content-length'] || 'unknown'
        }`
      );
      ...
      if (error.response) {
        ...
        console.log(
          `   Response body: ${error.response.data?.substring(0, 200)}...`
        );
      }
  ...
  extractProductInfo(data, clientType) {
    try {
      ...
      const originalPrice =
        product.originalPrice || product.regularPrice || price;
  ...
  async runTests() {
    ...
    console.log(
      `📦 대상 상품: ${this.baseUrl}/${this.storeId}/products/${this.productId}`
    );
    console.log(
      `🔌 API 엔드포인트: ${this.apiUrl}/channels/${this.channelId}/products/${this.productId}`
    );
    ...
    console.log(
      `   HttpClient 성공: ${
        testSummary.summary.httpClientSuccess ? '✅' : '❌'
      }`
    );
    console.log(
      `   CurlHttpClient 성공: ${
        testSummary.summary.curlHttpClientSuccess ? '✅' : '❌'
      }`
    );
