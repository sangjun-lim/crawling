...
class NaverStoreScraper extends BaseScraper {
  ...
  async searchStores(keyword, maxResults = null) {
    try {
      ...
      const coords = await CoordinateUtils.getLocationCoordinates(keyword, this.httpClient);
      ...
      return {
        ...
        stores: limitedStores.map((store, index) => ({ ...store, rank: index + 1 }))
      };
  ...
  async fetchStoresFromNaverMap(keyword, coords = null) {
    ...
    try {
      console.log(`네이버 지도 GraphQL API - ${this.config.pagination.maxPages}페이지 조회 시작...`);
      ...
      for (let page = 1; page <= this.config.pagination.maxPages; page++) {
        const displaySize = getDisplaySize(page, this.config.pagination.maxPages);
        const start = (page - 1) * this.config.pagination.pageSize + 1;
        const adStart = (page - 1) * this.config.pagination.adPageSize + 1;
        
        console.log(`페이지 ${page}/${this.config.pagination.maxPages} 요청 중... (start: ${start}, display: ${displaySize})`);
        
        const category = this.categoryDetector.detectCategory(keyword);
        ...
        const pageStores = this.responseParser.parseStoresFromGraphQLResponse(response.data, page);
        
        if (pageStores.length === 0) {
          console.log(`페이지 ${page}에서 더 이상 데이터가 없습니다. 조회 종료.`);
          break;
        }
        
        allStores.push(...pageStores);
        console.log(`페이지 ${page} 완료: ${pageStores.length}개 매장 추가 (총 ${allStores.length}개)`);
        ...
        if (pageStores.length < displaySize) {
          ...
          console.log(`페이지 ${page}에서 ${pageStores.length}개 매장 조회 (예상 크기: ${displaySize}개 미만). 조회 종료.`);
          break;
        }
      ...
      if (error.response?.data) {
        ...
        try {
          ...
          const errorFile = path.join(logDir, `error_response_${keyword.replace(/\s+/g, '_')}_${timestamp}.json`);
  ...
  detectCategory(keyword) {
    return this.categoryDetector.detectCategory(keyword);
  }

  buildGraphQLPayload(keyword, display, start, adStart, coords = null) {
    const category = this.categoryDetector.detectCategory(keyword);
    return this.graphQLBuilder.buildPayload(category, keyword, display, start, adStart, coords);
  }
}
