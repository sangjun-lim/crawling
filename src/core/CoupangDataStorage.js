...
class CoupangDataStorage {
  ...
  async saveVendorAsCSV(results, filename) {
    ...
    const csvData = validResults.map((result) => {
      ...
      return {
        ...
        품질배지: data.qualitySellerBadgeDto
          ? JSON.stringify(data.qualitySellerBadgeDto)
          : '',
    ...
    console.log(
      `   총 ${validResults.length}개 벤더 정보 저장 (필터링 전: ${successfulResults.length}개)`
    );
  ...
  async saveVendorToDatabase(results) {
    ...
    console.log(
      `   ${results.length}개 벤더 데이터를 임시로 JSON으로 저장합니다.`
    );
  ...
  async saveCombinedAsCSV(results, filename) {
    ...
    const csvData = results.map((row) => {
      const baseData = {
        ...
        품질배지: row.qualitySellerBadgeDto
          ? JSON.stringify(row.qualitySellerBadgeDto)
          : '',
  ...
  async saveCombinedToDatabase(results) {
    ...
    console.log('⚠️  데이터베이스 저장 기능은 아직 구현되지 않았습니다.');
    console.log(
      `   ${results.length}행 통합 데이터를 임시로 JSON으로 저장합니다.`
    );
    return await this.saveCombinedAsJSON(results, 'combined_temp');
  }

  // 점진적 저장 (배치)
  async saveIncrementalBatch(
    results,
    batchIndex,
    sessionId,
    type = 'combined'
    ...
    const batchFilename = `${sessionId}_batch_${String(batchIndex).padStart(
      6,
      '0'
    )}`;
    ...
    console.log(
      `✅ 배치 ${batchIndex} 저장: ${results.length}행 → ${filePath}`
    );
    return filePath;
  }
  ...
  async mergeBatches(sessionId, finalFilename = null) {
    ...
    const mergedFilename =
      finalFilename || `coupang_combined_merged_${timestamp}`;
